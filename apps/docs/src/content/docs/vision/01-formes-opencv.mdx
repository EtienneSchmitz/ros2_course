...
et complétez `on_image` avec votre détection :
...
def on_image(self, msg):
    ...
    for cnt in contours:
        if not (500 < cv.contourArea(cnt) < 200_000):
            continue
        centre = centroide(cnt)
        if centre is None:
            continue
        x, y, z = self.backproject(*centre)        # cf. socle commun
        self.publier(nommer_forme(cnt), x, y, z)   # pose 2D, orientation identité
```
...
Vous devez voir défiler des `Detection` avec un `class_id` (`triangle`, `carre`,
`cercle`) et une `pose` non nulle. **C'est votre premier détecteur réel.**

<Aside type="caution" title="Pose 2D">
La `pose` publiée est **2D dégradée** (objet supposé à plat, orientation identité). Pour
une pose **6D**, voir l'atelier [ArUco](/vision/02-aruco/).
</Aside>
...
3. Pourquoi la `pose` publiée est-elle « 2D dégradée » ?
