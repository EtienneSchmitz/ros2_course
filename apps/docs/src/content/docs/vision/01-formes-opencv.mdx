...
(`triangle`, `carre`, `cercle`) et une `position` non nulle. **C'est votre premier
détecteur réel.**

<Aside type="caution" title="Position 2D">
La `position` publiée suppose l'objet **posé à plat** (back-projection sur le plateau,
sans orientation). Pour une pose **6D**, voir l'atelier [ArUco](/vision/02-aruco/).
</Aside>
