...
![Arbre des repères du SO-101 dans RViz : chaque `<joint>` relie deux `<link>` (base_link → shoulder_link → upper_arm_link → lower_arm_link → wrist_link → gripper_link), formant la chaîne cinématique](/img/so101-joints-frames.png)
